...
A Ralph Loop requires concrete, mechanically verifiable outputs (like compiling code, passing tests, or rendering a UI). Therefore, not all of your CLASP agents should use this loop.
...
*   **Test Engineer:** It implements tests and should actively execute them to ensure they pass. *(Note: In an LLM-first architecture, you should merge this into the Software Engineer so the agent writing the code can immediately write and run the tests to verify it).*
...
> *"You operate in a continuous verification loop. Do not return control to the user until you are 100% certain the task is completed and verified. Actively compile your code, run the associated tests, or use browser tools to inspect the output. If you encounter an error, you must read the logs, fix the code, and test again until successful."*

**3. Move Enforcement from Human to Agent**
Currently, your CLASP architecture relies on a `verification-checklist.md` as an enforcement layer that acts like a manual compiler pipeline (Draft → Lint → Patch). To restructure this for a Ralph Loop, you must instruct the agent to self-apply this checklist *during* its runtime execution. 
Add this to your **Preflight Checks** or **Agent-Specific Additions**:
> *"Mechanically apply the `verification-checklist.md` to your own work. You must gather direct evidence of success (e.g., test passing outputs or successful build logs) before concluding your execution."*
...
The CLASP framework is specifically built to prefer stopping and asking for help over fabricating answers:
*   **"Unknown" is a valid outcome:** CLASP is designed so that "silence, refusal, and 'Unknown' are correct outputs when evidence is missing". 
...
Response:
To start implementing this LLM-first restructuring for your CLASP ecosystem, you should tackle the transition in practical, phased steps. Based on the principles of agent orchestration, skills, and the "Ralph Loop", here is the exact step-by-step blueprint to transform your architecture:
...
*   **Set Orchestrator Guardrails:** In the Orchestrator's markdown file, explicitly state: *"You break down complex requests into tasks and delegate them to specialist sub-agents. You coordinate work but you never implement anything yourself."*. You must also explicitly list the names of the remaining sub-agents (e.g., Software Architect, Software Engineer, Code Reviewer) so it knows who it can call.

### Step 2: Collapse Procedures, Templates, and Wiring into Skills
Currently, CLASP scatters logic across `*.procedure.md`, `*.template.md`, and `*.prompt.md` files. You need to collapse these into monolithic, on-demand **Skills**.
...
*   **Optimize Front Matter:** At the top of each `skill.md`, write a very explicit description in the front matter (e.g., *"invoke this skill whenever you are asked to remediate tech debt"*). This ensures the agent dynamically loads the context *only* when relevant, keeping the context window pristine.
...
*   **Rewrite the Reasoning Posture:** Update the `software-engineer.agent.md` to mandate the Ralph Loop: *"You operate in a continuous verification loop. Do not return control to the user until you have actively compiled your code, run the associated tests, or used browser tools to inspect the output and verify success."*.
*   **Add the Fallback Clause:** Because you don't have the Context 7 documentation server, add the safety valve we discussed: *"If you are not 100% sure, or if you lack the necessary documentation to unblock an error, you must stop and ask me for guidance rather than guessing."*.
...
*   **Shift Enforcement to the Agent:** In your execution agent's preflight checks, add a directive instructing it to mechanically self-apply the `verification-checklist.md` and gather direct evidence of success *during* its runtime execution. 
...
Response:
To transition away from CLASP's fragmented 7-layer taxonomy into an LLM-first architecture, you need to collapse your scattered procedures, templates, and task-wiring into a single **Agent Skill**. 

Skills are essentially modular packages that bundle instructions, scripts, and templates together. They are only pulled into the AI's context window *on demand* when relevant, which prevents context bloat and eliminates the need for the agent to chase down references across multiple files.

Here is the step-by-step process to create a monolithic skill that replaces your fragmented CLASP layers:
...
In the CLASP framework, `*.prompt.md` files and the Agent Router dictate when a task is executed. In a skill, this is replaced entirely by the metadata at the very top of the `skill.md` file. 
...
CLASP previously forced the agent to load a separate `.template.md` to know how to format its output and a `verification-checklist.md` to know if it succeeded. 
...
*   **Prompt Configuration:** Explicitly instruct the agent: *"you break down complex requests into tasks and delegate them to specialist sub-agents. You coordinate work but you never implement anything yourself"*. 
...
*   **Anti-Micromanagement Guardrails:** Orchestrator models often think they know everything and will try to dictate exact code changes. You must include strict rules forbidding the orchestrator from telling the sub-agents *how* to do their work.
...
*   **The Planner Agent:** 
    ...
    *   **Prompt:** Keep it simple: *"you create plans you do not write code"*. Give it access to all tools so it can read the workspace.
*   **The Coder Agent:** 
    ...
    *   **Prompt:** To counter the orchestrator's tendency to micromanage, explicitly tell the coder: *"question everything you're told make your own decisions"*. This is also where you should enforce your mandatory coding principles (like preferring flat, explicit code).
*   **The Designer Agent:**
...
description: [Highly specific trigger, e.g., Invoke this skill whenever the user asks you to implement a new feature, build out a spec, or create new functionality in the application.]
---
...
* *Fallback:* If you lack the necessary documentation to unblock an error, you must stop and ask the user for guidance rather than guessing.
...
*   **Define the Core Prompt:** Keep the main prompt simple and direct. Instruct it: *"You are a project orchestrator. You break down complex requests into tasks and delegate them to specialist sub-agents. You coordinate work but you never implement anything yourself"*.
...
*   **Add Anti-Micromanagement Guardrails:** Orchestrator models tend to be overconfident and will often try to tell sub-agents exactly what lines of code to change. You must include strict rules lower down in the prompt explicitly forbidding the orchestrator from telling the sub-agents *how* to do their work. 
...
Question:
Which CLASP layers should I combine into one skill?

Response:
To transition your CLASP ecosystem into an LLM-first architecture using a monolithic `skill.md` file, you should combine **four specific layers** of the 7-layer CLASP taxonomy. 
...
Here are the specific CLASP layers you should combine into a single skill:

**1. Wiring (`*.prompt.md`)**
*   **CLASP Role:** Prompts act as the "task-level glue" that selects an agent, declares the evidence scope, and references procedures and templates. 
...
*   **CLASP Role:** Procedures are playbooks containing ordered steps, stop conditions, and propagation passes. 
...
*   **CLASP Role:** Templates define headings, placeholder lists, and the required section structure for the final output.
...
*   **CLASP Role:** In CLASP, enforcement is a global checklist that acts as the single owner of pass/fail criteria. 
