# the project AI-Harness Evaluation  
## Spezifikations- und Implementierungsplan

**Stand:** 2026-06-03  
**Ziel:** Vorher/Nachher-Bewertung von Änderungen an the project-Agenten, Prompts, Instructions, Skills und ähnlichen Harness-Artefakten.  
...
Der beste Startpunkt für the project ist **kein großes Benchmark-System**, sondern eine kleine, harte, gepaarte Eval-Suite:
...
## 1. Verifizierter the project-Kontext

Die folgenden Punkte sind aus dem Repository verifiziert und werden als Planbasis genutzt.

### 1.1 Repository-Struktur

the project ist laut Root-Agent-Vertrag ein **Dividend-Capture Decision System für US-Dividendenaktien, primär BDCs**. Die wichtigsten Schichten sind:

| Bereich | Pfad |
|---|---|
| Backend | `the-project-api/` |
| Frontend | `the-project-web/` |
...
the project hat bereits verifizierte Validierungspfade:

```text
Python lint:  ruff check the-project-api/app/
Python types: cd the-project-api && pyright app/
Python tests: cd the-project-api && python -m pytest tests/ -x -q

TypeScript lint:  cd the-project-web && eslint src/
TypeScript types: cd the-project-web && tsc --noEmit
TypeScript tests: cd the-project-web && npm test -- --run
```
...
the project hat bereits Hook-Konfigurationen unter `.github/hooks/hooks.json`:
...
Gemessen wird die Qualität des **the project AI Harness** nach Änderungen an:
...
|---:|---|---:|---:|---|---|
| 1 | Deterministische Gates | Niedrig | Sehr hoch | Ja | Nutzt vorhandene the project-Lint/Test/Freshness-Befehle. Billig, objektiv, reproduzierbar. |
...
| 6 | Hook-basierter Eval-Modus | Mittel | Hoch | Ja, nach MVP | the project hat bereits Hooks. Damit lassen sich Tool-Calls, Stop-Verhalten und Verifikationsnachweise erfassen. |
...
| 10 | Trace-Graph-/Dominator-Analyse | Hoch | Hoch bei UI/Computer-Use | Nicht MVP | Für the project aktuell zu schwergewichtig. Optional für spätere Agent-Trace-Analyse. |
| 11 | AgentEval-/DAG-Failure-Attribution | Hoch | Hoch | Nicht MVP | Gut für große Trace-Korpora, aber nicht für den Start. |
| 12 | Vollständige VS-Code-Runner-Automation | Hoch | Sehr hoch | Nicht MVP | Realistisch, aber unnötig teuer für die erste the project-Version. |
...
**Warum `.github/evals/`?**  
the project hält Copilot-bezogene Agenten, Prompts, Instructions, Skills, Hooks und Schemas bereits unter `.github/`. Der Eval-Katalog gehört als Copilot/Agent Harness-nahe Harness-Fläche ebenfalls dort hin.
...
|---|---|
| `the-project-api/app/**` | `ruff check the-project-api/app/` |
| `the-project-api/app/**` | `cd the-project-api && pyright app/` |
| `the-project-api/tests/**` | `cd the-project-api && python -m pytest tests/ -x -q` |
| `the-project-web/src/**` | `cd the-project-web && eslint src/` |
| `the-project-web/src/**` | `cd the-project-web && tsc --noEmit` |
| `the-project-web/src/**` | `cd the-project-web && npm test -- --run` |
...
def score_trial(task, output_path, changed_files):
    ...
    if touches_backend(changed_files):
        result["ruff"] = run("ruff check the-project-api/app/")
        result["pyright"] = run("cd the-project-api && pyright app/")

    if touches_frontend(changed_files):
        result["eslint"] = run("cd the-project-web && eslint src/")
        result["tsc"] = run("cd the-project-web && tsc --noEmit")
...
2. the project-Kontext ist Primärquelle für Repo-spezifische Behauptungen.
...
**Ziel:** Bestehende the project-Hooks nutzen, aber nur im Eval-Modus aktiv zusätzliche Logs schreiben.

#### 5.1 Environment Flags

```powershell
$env:THE_PROJECT_AI_EVAL="1"
$env:THE_PROJECT_AI_EVAL_EXPERIMENT="2026-06-03-engineer-agent-v2"
$env:THE_PROJECT_AI_EVAL_VARIANT="candidate"
$env:THE_PROJECT_AI_EVAL_TASK="smoke-doc-001"
$env:THE_PROJECT_AI_EVAL_TRIAL="1"
```
...
- Ohne `THE_PROJECT_AI_EVAL=1` bleibt Verhalten unverändert.
- Mit `THE_PROJECT_AI_EVAL=1` entsteht ein Event-Log.
...
- the project-eigene JSON-Ergebnisse bleiben Source of Truth.
...
- falsche the project-Dateien behauptet werden,
...
## 7. Empfohlene Metriken für the project
...
the project hat Hooks bereits. Deshalb ist ein einfacher JSONL-Eval-Modus günstiger als sofort Phoenix/LangSmith/OTel-Backend einzuführen.
...
Aber: Eine zusätzliche Dependency und ein zweites Konfigurationsmodell erhöhen Komplexität. Darum erst the project-eigene JSON-Ergebnisse als Source of Truth, Promptfoo später als Adapter.
...
**Warum:**  
the project hat bereits gute Bausteine: Validierungskommandos, Agent Harness-Freshness-Skripte, Agent-/Skill-/Prompt-Struktur und Hooks. Der schnellste Weg zu verwertbaren Ergebnissen ist, diese vorhandenen Flächen zu nutzen, statt eine neue Plattform daneben zu bauen.
